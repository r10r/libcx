...
static uint32_t
rotl32(uint32_t n, uint8_t bits)
{
	return (n << bits) | (n >> (32 - bits));
}

static uint32_t
change_endian32(uint32_t x)
{
...
void
sha1_nextBlock(sha1_ctx_t* state, const void* block)
{
	uint32_t a[5];
	uint32_t w[16];
	uint32_t temp;
...
void
sha1_lastBlock(sha1_ctx_t* state, const void* block, uint16_t length)
{
...
void
sha1_ctx2hash(void* dest, sha1_ctx_t* state)
{
#if defined LITTLE_ENDIAN
	uint8_t i;
	for (i = 0; i < 5; ++i)
 ...
 */
void
sha1(void* dest, const void* msg, uint32_t length)
{
	sha1_ctx_t s;

	sha1_init(&s);
	while (length & (uint32_t)(~0x0001ff))  /* length>=512 */
	{
		sha1_nextBlock(&s, msg);
		msg = (uint8_t*)msg + SHA1_BLOCK_BITS / 8; /* increment pointer to next block */
		length -= SHA1_BLOCK_BITS;
	}
	sha1_lastBlock(&s, msg, (uint16_t)length);
