...
RPC_Request_create_error(Request* request, RPC_Error err, const char* message)
{
	...
	RPC_Result_error_with_message(&rpc_request.result, err, message);
...
deserialize_id(RPC_Request* request, json_t* id_json)
{
	...
	else
	{
		...
		{
			RPC_Result_error_with_message(&request->result, CX_RPC_ERROR_INVALID_REQUEST,
...
Request_from_json(RPC_Request* request, json_t* request_json)
{
	...
	if (unpack_status != 0)         /* success */
	{
		...
		RPC_Result_error_with_message(&request->result, CX_RPC_ERROR_INVALID_REQUEST, error.text);
		return -1;
	}
	else
	{
